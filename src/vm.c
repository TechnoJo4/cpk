...
typedef void (*F)(K x, K y, K* c, B* b);
...
void vm(K x, K y, K* c, B* b) {
    ...
    C32(OP_CDBV) retail dyads[o-OP_CDBV](x, y, c, b);
    C32(OP_CMBV) retail monads[o-OP_CMBV](x, y, c, b);
    C32(OP_xKBV) vmtail(x, y);//nyi
    C32(OP_CDBX) y = x; x = ((K**)c[-1])[1][*(b++)]; retail dyads[o-OP_CDBX](x, y, c, b);
    }
}
